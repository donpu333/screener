...
class MarketWebSocket {
    ...
    connectBinance() {
        ...
        // Убираем лимит - подключаем все пары
        if (binanceSymbols.length > 0) {
            const streamUrl = `wss://fstream.binance.com/stream?streams=${binanceSymbols.join('/')}`;
            console.log(`🔌 Binance WebSocket: ${binanceSymbols.length} потоков`);
            
            this.wsBinance = new WebSocket(streamUrl);
            this.setupBinanceHandlers();
        } else {
            console.log('⚠️ Нет пар Binance для WebSocket');
        }
    }
    ...
    setupBybitHandlers(symbols, interval) {
        this.wsBybit.onopen = () => {
            ...
            // Подписываемся на kline для всех символов
            const args = symbols.map(s => `${s}.${interval}`);
            ...
            this.wsBybit.send(JSON.stringify(subscribeMsg));
