...
class BinanceAdapter extends BaseAdapter {
    ...
    async fetchAllTickers(marketType = 'futures') {
        ...
        data.forEach(t => {
            ...
            // 🔥 ИСПРАВЛЕНО: tradesCount как ЧИСЛО
            const tradesCount = parseInt(t.count) || 0;
            ...
            result[symbol] = {
                ...
                tradesCount: tradesCount,  // 🔥 Теперь число
                openPrice24h: parseFloat(t.openPrice || t.open),
                lastUpdate: Date.now()
            };
        });
    ...
    async fetchKlines(symbol, interval, limit = 500, endTime = null, marketType = 'futures') {
        ...
        return data
            .map(d => ({
                ...
                trades: parseInt(d[8]) || 0  // 🔥 Добавлено количество сделок в свече
            }))
...
class BybitAdapter extends BaseAdapter {
    ...
    async fetchAllTickers(marketType = 'futures') {
        ...
        data.result.list.forEach(t => {
            ...
            // 🔥 Bybit не отдаёт количество сделок в tickers, ставим 0
            result[symbol] = {
                symbol: symbol,
                exchange: this.name,
                marketType: marketType,
                ...
                tradesCount: 0,  // 🔥 Bybit — нет данных
                openPrice24h: parseFloat(t.prevPrice24h || t.openPrice),
                lastUpdate: Date.now()
            };
        });
    ...
    async fetchKlines(symbol, interval, limit = 500, endTime = null, marketType = 'futures') {
        ...
        return data.result.list
            .map(d => ({
                ...
                close: parseFloat(d[4]),
                volume: parseFloat(d[5]),
                trades: 0  // Bybit не отдаёт количество сделок в kline
            }))
            .filter(c => c.volume > 0 && !isNaN(c.open) && !isNaN(c.high) && !isNaN(c.low) && !isNaN(c.close))
            .reverse();
    }
