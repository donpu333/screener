...
addTimeframeSelector() {
    ...
    // === ИСПРАВЛЕНО: переключение таймфрейма ===
    selectorContainer.addEventListener('click', async (e) => {
        e.stopPropagation();
        
        // Создаём простое меню выбора
...
// === НОВЫЙ МЕТОД ДЛЯ ПЕРЕКЛЮЧЕНИЯ ТАЙМФРЕЙМА ===
...
setData(data) {
    ...
    // === УСТАНАВЛИВАЕМ КОЛБЭК ДЛЯ СМЕНЫ ТАЙМФРЕЙМА ===
    if (!this.onIntervalChangeCallback) {
...
openPopup() {
    ...
    // Клик по оверлею (вне popupContainer) — закрыть
