...
this.setupScrollToTop();
    ...
    // Автообновление тикеров каждые 60 секунд
// Автообновление тикеров каждые 60 секунд (без мерцания)
this._autoRefresh = setInterval(async () => {
    const before = this.marketTickers.size;
    await this.loadMarketDataSilent();
    if (this.marketWS) this.marketWS.resubscribe();
    console.log(`🔄 Фон: тикеров ${before} → ${this.marketTickers.size}`);
}, 60000);
