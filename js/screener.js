...
this.setupScrollToTop();
  ...
  this._autoRefresh = setInterval(async () => {
    ...
    // 🔥 убрать эту строку: if (this.marketWS) this.marketWS.resubscribe();
    console.log(`🔄 Фон: обновлено`);
}, 60000);

// Фоновое обновление + проверка аномалий каждые 60 секунд
...
this._autoRefresh = setInterval(async () => {
    ...
    this.checkAnomalies();
    this.updateCounts();
    console.log('🔄 Фон: обновлено, аномалии проверены');
}, 60000);
...
if (flagFilterBtn) {
  ...
  showFlagFilterPopup(e, button) {
    ...
    // 🔥 Кнопка "Все флаги" — четыре маленьких флажка разных цветов
    const allBtn = document.createElement('div');
    allBtn.className = 'flag-all' + (this.state.flagFilter === 'all' ? ' active' : '');
    allBtn.title = 'Все помеченные';
    allBtn.style.cssText = 'display:flex; gap:2px; align-items:center; justify-content:center;';
    allBtn.innerHTML = `
        <span style="display:inline-block;width:8px;height:12px;background:#e74c3c;border-radius:2px;"></span>
        <span style="display:inline-block;width:8px;height:12px;background:#3498db;border-radius:2px;"></span>
        <span style="display:inline-block;width:8px;height:12px;background:#2ecc71;border-radius:2px;"></span>
        <span style="display:inline-block;width:8px;height:12px;background:#f1c40f;border-radius:2px;"></span>
    `;
    ...
    resetBtn.className = 'flag-all';
    resetBtn.textContent = '×';
    resetBtn.title = 'Сбросить фильтр флагов';
    resetBtn.addEventListener('click', (ev) => {
        ev.stopPropagation();
        this.state.flagFilter = 'show_all';
        popup.remove();
        this.filterByFlag('show_all');
    });
...
filterByFlag(flagColor) {
    ...
    cards.forEach(card => {
        if (flagColor === 'all') {
            // Показать все карточки с любым флагом
            ...
            // Показать только с конкретным цветом
            const flagEl = card.querySelector('.flag');
            if (flagEl && flagEl.classList.contains(`flag-${flagColor}`)) {
                card.style.display = '';
            } else {
  ...
  async loadMarketData() {
   ...
   updateCounts() {
    ...
    if (this.elements.counts.gainers) {
        this.elements.counts.gainers.textContent = marketItems.filter(t => t.change24h >= 3).length;
    }
    if (this.elements.counts.losers) {
        this.elements.counts.losers.textContent = marketItems.filter(t => t.change24h <= -3).length;
    }
...
checkAnomalies() {
    ...
    const settings = this.alertManager.autoSettings;
    const intervalMinutes = {
        '1m': 1, '3m': 3, '5m': 5, '15m': 15,
        '1h': 60, '4h': 240, '1d': 1440
    };
    const intervalMin = intervalMinutes[settings.interval] || 5;
    
    for (const [key, ticker] of this.marketTickers) {
        const prev = this._prevTickerValues.get(key);
        if (!prev) continue;
        
        // Сколько минут прошло с предыдущего замера
        const elapsedMinutes = (Date.now() - prev.timestamp) / 60000;
        if (elapsedMinutes < intervalMin) continue;
        
        // Рост цены
        if (settings.priceUp > 0 && prev.price > 0) {
            const priceChange = ((ticker.price - prev.price) / prev.price) * 100;
            if (priceChange > settings.priceUp) {
                this.alertManager.addHistoryEntry({
                    symbol: ticker.symbol,
                    type: 'price_up',
                    message: `+${priceChange.toFixed(2)}% за ${intervalMin}м`
                });
                if (settings.sound) Utils.playBeep();
                Utils.showNotification(`🚀 ${ticker.symbol}`, `Рост +${priceChange.toFixed(2)}% за ${intervalMin}м`);
            }
        }
        
        // Падение цены
        if (settings.priceDown > 0 && prev.price > 0) {
            const priceChange = ((ticker.price - prev.price) / prev.price) * 100;
            if (priceChange < -settings.priceDown) {
                this.alertManager.addHistoryEntry({
                    symbol: ticker.symbol,
                    type: 'price_down',
                    message: `${priceChange.toFixed(2)}% за ${intervalMin}м`
                ...
                if (settings.sound) Utils.playBeep();
                Utils.showNotification(`📉 ${ticker.symbol}`, `Падение ${priceChange.toFixed(2)}% за ${intervalMin}м`);
            }
        }
        
        // Рост объёма
        if (settings.volUp > 0 && prev.volume > 0) {
            const volChange = ((ticker.volume24h - prev.volume) / prev.volume) * 100;
            if (volChange > settings.volUp) {
                this.alertManager.addHistoryEntry({
                    symbol: ticker.symbol,
                    type: 'volume_surge',
                    message: `Объём +${volChange.toFixed(0)}% за ${intervalMin}м`
                });
                if (settings.sound) Utils.playBeep();
            }
        }
        ...
        // Рост сделок
        if (settings.tradesUp > 0 && prev.trades > 0) {
            const tradesChange = ((ticker.tradesCount - prev.trades) / prev.trades) * 100;
            if (tradesChange > settings.tradesUp) {
                this.alertManager.addHistoryEntry({
                    symbol: ticker.symbol,
                    type: 'trades_surge',
                    message: `Сделки +${tradesChange.toFixed(0)}% за ${intervalMin}м`
                });
                if (settings.sound) Utils.playBeep();
            }
        }
        
        // Обновляем только когда прошёл интервал
        if (elapsedMinutes >= intervalMin) {
            this._prevTickerValues.set(key, {
                price: ticker.price,
                volume: ticker.volume24h,
                trades: ticker.tradesCount,
                timestamp: Date.now()
            });
        }
    }
...
loadFlags() {
    try {
        this.state.flags = JSON.parse(localStorage.getItem('crypto_flags')) || {};
    } catch (e) {
        this.state.flags = {};
    }
}
